...
**Gauss–Hermite with 15 nodes is ~30× more accurate than the current default while using half the
geometries**, because cost scales as n³: 15³ = 3375 versus 19³ = 6859. That is the recommended change.
Note n = 7 is *not* enough — the oscillation needs the higher-order rule — so the naive "few nodes
suffice" intuition also fails here.
